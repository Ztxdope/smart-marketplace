          ...
          child: Wrap(
            children: [
              const Padding(
                ...
                child: Center(child: Text("Ubah Status Barang", style: TextStyle(fontWeight: FontWeight.bold))),
              ),
              const Divider(),
              ListTile(
                leading: const Icon(Icons.check_circle_outline, color: Colors.green),
                title: const Text("Tandai Tersedia"),
                trailing: currentStatus == 'Tersedia' ? const Icon(Icons.check, color: Colors.blue) : null,
                onTap: () => _updateStatus(productId, 'Tersedia'),
              ),
              ListTile(
                leading: const Icon(Icons.timelapse, color: Colors.orange),
                title: const Text("Tandai Sedang Diproses"),
                trailing: currentStatus == 'Sedang Diproses' ? const Icon(Icons.check, color: Colors.blue) : null,
                onTap: () => _updateStatus(productId, 'Sedang Diproses'),
              ),
              ListTile(
                leading: const Icon(Icons.monetization_on, color: Colors.grey),
                title: const Text("Tandai Terjual"),
                trailing: currentStatus == 'Terjual' ? const Icon(Icons.check, color: Colors.blue) : null,
                onTap: () => _updateStatus(productId, 'Terjual'),
              ),
            ],
          ),
        );
        ...
        builder: (context, snapshot) {
          ...
          if (products.isEmpty) {
            return Center(
              child: Column(
                ...
                children: [
                  Icon(FluentIcons.box_dismiss_24_regular, size: 60, color: Colors.grey[300]),
          ...
          return ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              // Normalisasi status (jika null atau bahasa inggris, anggap Tersedia)
              ...
              return Slidable(
                ...
                // GESER KANAN -> EDIT HALAMAN PENUH
                startActionPane: ActionPane(
                  motion: const ScrollMotion(),
                  children: [
                    SlidableAction(
                      onPressed: (_) {
                        Navigator.push(context, MaterialPageRoute(builder: (_) => EditProductPage(product: item)));
                      },
                ...
                endActionPane: ActionPane(
                  ...
                  children: [
                    SlidableAction(
                      onPressed: (_) => _deleteProduct(item['id']),
